...
func (p *PostProcessor) PostProcess(
	...
	case "pgzip":
		...
		defer output.Close()
	case "zstd":
		ui.Say(fmt.Sprintf("Using zstd compression for %s",
			target))
        output, err = makeZstdWriter(outputFile)
		if err != nil {
			return nil, false, false, fmt.Errorf(errTmpl, p.config.Algorithm, err)
		}
		defer output.Close()
...
func makeZstdWriter(output io.WriteCloser) (io.WriteCloser, error) {
